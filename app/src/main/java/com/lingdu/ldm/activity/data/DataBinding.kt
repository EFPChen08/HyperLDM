...
object DataBinding {
    ...
    class Binding(private val defValue: () -> Any, val recvCallbacks: (View, Int, Any) -> Unit) {
         ...
         * <p>
         * Please use {@link com.lingdu.ldm.activity.data.DataBinding.Binding#getRecv(int)}
         */
        inner class Recv(private val flags: Int, private val defValue: () -> Any) {
            ...
            fun recv(any: Any) {
                // 【修复核心】：在使用 mView 之前，先检查它是否已经初始化
                if (::mView.isInitialized) {
                    recvCallbacks(mView, flags, any)
                } else {
                    // 可选：如果 View 还没准备好，就忽略这次更新，防止崩溃
                    // android.util.Log.w("DataBinding", "View not initialized, skipping update.")
                }
            }
        }
    }
